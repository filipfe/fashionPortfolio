...
export default function Signup() {
    ...
    return (
        <section className='padding-x padding-y bg-white flex items-center sm:h-screen justify-center lg:justify-start lg:grid lg:grid-cols-2'>
...
function Form() {
    ...
    return (
        <form className='flex flex-col bg-white gap-6 my-4 xl:mt-6' onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:gap-0 md:justify-between">
                <button type="submit" className={`${buttonStyles} mt-6 px-10 font-medium max-w-max`}>Create an account</button>
...
const Activation = () => {
    ...
    return (
        <>
            ...
            {alert && alert !== 'loading' ? <div className={`text-xl ${alert === "Successfully activated" ? 'text-green-500' : 'text-red-500'}`}>{alert}</div> : <></>}
            <Link className={`${buttonStyles} mt-6 px-10 max-w-max font-medium`} to='/login'>Log in</Link>
        </>
    )
}

export const inputStyles = 'bg-[#FBFBFB] py-2 px-6 sm:px-10 max-w-full font-medium rounded-md placeholder:font-normal border-[1px] border-[#E6E6E6] outline-none'