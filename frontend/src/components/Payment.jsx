...
function Form() {
    ...
    const Menu = () => {
        ...
        return (
            <div className="relative flex items-center">
                <input className={`${inputStyles} w-full cursor-default`} onClick={() => setActive(prev => !prev)} value={method} required type='text' name='payment' placeholder="Payment method" />
                ...
                {active ? <div className='flex flex-col absolute bottom-0 rounded overflow-hidden translate-y-full right-0 left-0 border-x-[1px] border-[#E6E6E6]'>
                    {methods.map(method => <div onClick={() => setMethod(method)} className="odd:bg-[#FBFBFB] bg-white border-b-[1px] border-[#E6E6E6] w-full py-3 px-6" key={method}>{method}</div>)}
                </div> : <></>}
            </div>
        )
    }
    ...
    return (
        <div className='sm:border-[1px] sm:border-[#E6E6E6] sm:p-6 md:p-10 sm:rounded sm:mt-6 sm:flex-1'>
            ...
            <form className="flex flex-col gap-6 my-4 sm:gap-8">
                <Menu />
                <div className='bg-[#FBFBFB] rounded-md font-medium flex justify-center items-center min-h-[1in] border-[#E6E6E6] border-[1px]'>Add method</div>
